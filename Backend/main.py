    ...
    try:
        ...
        for raw_tx in data:
            ...
            if merchant_particulars:
                label_entry = db.execute(select(DB_models.label).where(DB_models.label.particulars == merchant_particulars)).first()
                if label_entry:
                    merchant_id = label_entry.merchant_id
                    merchant = db.execute(select(DB_models.merchant).where(DB_models.merchant.id == merchant_id)).first()
            ...
            new_tx = DB_models.transaction(
                ...
                category_id=category_id,  # Leaving blank unless you want to pull it from the merchant table
                tx_type=tx_type,
                amount=amount,
            ...
            new_raw_tx = DB_models.raw_transaction(**raw_tx.model_dump(exclude_unset=True), tx_id=new_tx.id)
            db.add(new_raw_tx)

        # Commit everything as a single batch
        db.commit()
        return models.api_response(success=True, message=f"{len(data)} transactions processed and added successfully.")
